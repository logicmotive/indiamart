...
def sync_india_mart_lead(from_date, to_date):
    try:
        frappe.set_user("Administrator")  # Temporarily set user to Administrator
        ...
        if res.text:
            try:
                ...
                frappe.logger().error(f"Failed to parse API response as JSON. Raw response: {res.text[:1000]}")
                return

            frappe.logger().info(f"API Response: {json.dumps(response_data, indent=2)}")
            ...
            if isinstance(response_data, dict):
                if response_data.get("CODE") == 429:
                    ...
                    frappe.logger().error(f"Response is a dictionary, expected a list. Response: {json.dumps(response_data, indent=2)}")
                    return
            elif not isinstance(response_data, list):
                frappe.logger().error(f"Unexpected response type. Expected list, got {type(response_data)}. Response: {json.dumps(response_data, indent=2)}")
                return
            ...
            for row in response_data:
                try:
                    ...
                    frappe.logger().error(f"Error processing lead: {str(e)}")
                    leads_failed += 1
...
def test_manual_lead_creation():
    ...
    frappe.logger().info(f"Manual lead creation result: {result}")
